...
class PlanProblem:
    ...
    def helpfulActions(self, start):
        plan = self.extractRelaxedPlan(start)
        if plan is None:
            return []
        (relaxedPlan, firstGoals) = plan
        #return relaxedPlan[0] #More restrictive version described in paper
        return [action for action in self.actionInstances if start.check(action.preconditions) and any([effect in firstGoals for effect in action.positiveEffects])]
